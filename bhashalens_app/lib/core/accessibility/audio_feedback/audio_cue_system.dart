...
import 'package:bhashalens_app/features/splash_onboarding/data/models/audio_feedback_config.dart';
...
class SystemAudioCuePlayer implements AudioCuePlayer {
  ...
  Future<void> setVolume(double volume) async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('Failed to set audio cue volume: $e');
      }
    }
  }
...
class FallbackAudioCuePlayer implements AudioCuePlayer {
  ...
  @override
  Future<void> setVolume(double volume) async {
    // Volume control not supported for system sounds
  }
...
class AudioCueSystem {
  ...
  Future<void> handleError(String errorType, {String? customMessage}) async {
    ...
    await _audioFeedbackService.announceError(errorType,
        customMessage: customMessage);
  }
  ...
  bool _pausedByVoiceNavigation = false;
  ...
  Future<void> coordinateWithVoiceNavigation(bool voiceNavigationActive) async {
    if (voiceNavigationActive) {
      ...
      if (_audioFeedbackService.isSpeaking) {
        ...
        _pausedByVoiceNavigation = true;
      }
    } else {
      // Restore normal audio cue volume
      await setCueVolume(1.0);

      // Resume TTS if it was paused by this system
      if (_pausedByVoiceNavigation) {
        await _audioFeedbackService.resumeSpeech();
        _pausedByVoiceNavigation = false;
      }
    }

    if (kDebugMode) {
      print(
          'Audio cue coordination: voice navigation ${voiceNavigationActive ? 'active' : 'inactive'}');
    }
  }
    ...
    _backgroundNotificationTimer = Timer.periodic(
      ...
      (timer) {
        if (_backgroundTtsEnabled && _backgroundNotifications.isNotEmpty) {
          unawaited(announceBackgroundNotifications());
        }
      },
    );
  }
  ...
  static AudioCueSystem createDefault(
  ...
  static AudioCueSystem createWithSystemAudio(
      AudioFeedbackService audioFeedbackService) {
    return AudioCueSystem(
      audioFeedbackService: audioFeedbackService,
...
class AudioCueSystemFactory {
  static AudioCueSystem create({
    ...
    final cuePlayer =
        useSystemAudio ? SystemAudioCuePlayer() : FallbackAudioCuePlayer();
